...
export class ReviewService {
    ...
    async getUserReviews(userId: number): Promise<{
        ...
        return {
            product_reviews: productReviews
                ...
                .map(r => this.toReviewResponse(r, "product")),
            store_reviews: storeReviews
                .filter(r => r.store_id !== null)
                .map(r => this.toReviewResponse(r, "store"))
        };
    }
    ...
    async canReviewStore(userId: number, storeId: number): Promise<CanReviewResponse> {
        ...
        // Check if user has purchased from this store (delivered order)
        const purchasedOrder = await this.orderRepository.findOne({
            where: { user_id: userId, store_id: storeId, status: "delivered" }
        });

        if (!purchasedOrder) {
            return {
                can_review: false,
                reason: "You can only review stores you have purchased from",
                has_existing_review: false,
                existing_review_id: null
            };
        }
    ...
    async createProductReview(userId: number, data: CreateProductReviewRequest): Promise<ReviewResponse> {
        // Validate
        this.validateReviewData(data.rating, data.title, data.description);
        ...
        // Update product rating (via store since product doesn't have rating fields)
    ...
    private validateReviewData(rating: number, title: string, description: string): void {
    ...
    private async updateStoreRating(storeId: number): Promise<void> {
        // Get all store reviews + product reviews for this store
        const storeReviews = await this.reviewRepository.find({
            where: { store_id: storeId }
        });
        ...
        if (productIds.length > 0) {
            productReviews = await this.reviewRepository
                .createQueryBuilder("review")
    ...
    private toReviewResponse(review: Review, type: "product" | "store"): ReviewResponse {
        ...
        if (type === "product") {
            target = {
                id: review.product?.id || review.product_id || 0,
                name: review.product?.title || "Unknown Product"
            };
        } else {
            target = {
                id: review.store?.id || review.store_id || 0,
                name: review.store?.name || "Unknown Store"
            };
        }
